...
// Ensure data/.env is loaded before boot hydration runs (env.js also loads this,
// but vault must not depend on require order when used from scripts).
dotenv.config({ path: ENV_FILE });
try { dotenv.config(); } catch (_) { /* dotenv optional */ }

const vault = require('./controltower-vault');
const { saveEnvCache, loadEnvCache } = require('./vaultEnvCache');
...
// Provider-specific field mappings (same convention as FlyApp / ControlTower SDK).
const PROVIDER_FIELD_DEFAULTS = {
  'openai.api_key': { env: 'OPENAI_API_KEY' },
  'openai.default_model': { env: 'OPENAI_MODEL', preserveExisting: true }
};
...
function mappingFor(providerKey, field, userMapping) {
  ...
  const mapKey = `${providerKey}.${field}`.toLowerCase();
  const explicit = userMapping[mapKey] || PROVIDER_FIELD_DEFAULTS[mapKey];
...
async function hydrate({ silent = false } = {}) {
  ...
  if (!errors.length && total > 0) saveEnvCache();
...
function isRateLimitError(err) {
  ...
  return msg.includes('(429)') || /too many login attempts/i.test(msg);
}
...
async function hydrateWithRetry(opts = {}) {
  ...
  const cached = loadEnvCache({ onlyMissing: true });
  if (cached.applied) {
    console.warn(`[vault] loaded ${cached.applied} cached env value(s) from ${cached.savedAt || 'prior boot'}`);
  }
  const hasBaseCreds = !!(process.env.SP_API_LWA_CLIENT_ID && process.env.SP_API_LWA_CLIENT_SECRET);

  let last = await hydrate({ ...opts, silent: hasBaseCreds });
  if (!last.errors || !last.errors.length) return last;

  if (last.errors.some((e) => isRateLimitError(e.error))) {
    if (process.env.SP_API_LWA_CLIENT_ID && process.env.SP_API_LWA_CLIENT_SECRET) {
      console.warn('[vault] ControlTower rate-limited (429) — continuing with cached credentials; vault refresh will retry in background');
      return { ...last, errors: [], fromCache: true };
    }
    loadEnvCache({ onlyMissing: false });
    if (process.env.SP_API_LWA_CLIENT_ID && process.env.SP_API_LWA_CLIENT_SECRET) {
      console.warn('[vault] ControlTower rate-limited (429) — loaded SP-API credentials from disk cache');
      return { ...last, errors: [], fromCache: true };
    }
    console.error('[vault] ControlTower rate-limited (429) — not retrying (each retry extends the lockout). Wait ~15 minutes, then restart once.');
    return last;
  }

  let attempt = 0;
  while (attempt < BOOT_RETRY_ATTEMPTS) {
    attempt += 1;
    const delay = Math.min(BOOT_RETRY_BASE_MS * 2 ** (attempt - 1), BOOT_RETRY_MAX_MS);
    const why = last.errors.map((e) => `${e.provider}: ${e.error}`).join('; ');
    console.warn(`[vault] boot hydration failed (attempt ${attempt}/${BOOT_RETRY_ATTEMPTS + 1}); retrying in ${Math.round(delay / 1000)}s — ${why}`);
    await sleep(delay);
    last = await hydrate({ ...opts, silent: true });
    if (!last.errors || !last.errors.length) return last;
    if (last.errors.some((e) => isRateLimitError(e.error))) {
      if (process.env.SP_API_LWA_CLIENT_ID && process.env.SP_API_LWA_CLIENT_SECRET) {
        console.warn('[vault] ControlTower rate-limited (429) — continuing with cached credentials');
        return { ...last, errors: [], fromCache: true };
      }
      loadEnvCache({ onlyMissing: false });
      if (process.env.SP_API_LWA_CLIENT_ID && process.env.SP_API_LWA_CLIENT_SECRET) {
  ...
  loadEnvCache({ onlyMissing: false });
  if (process.env.SP_API_LWA_CLIENT_ID && process.env.SP_API_LWA_CLIENT_SECRET) {
    console.warn('[vault] boot hydration failed — continuing with disk cache');
    return { ...last, errors: [], fromCache: true };
  }
...
function bootReady() {
  if (!_readyPromise) {
    _readyPromise = hydrateWithRetry({ silent: process.env.CT_VAULT_QUIET === '1' });
  }
...
if (REFRESH_MIN > 0) {
  setInterval(() => {
    vault.invalidate();
    hydrate({ silent: true }).then((r) => {
      if (!r.errors || !r.errors.length) saveEnvCache();
    }).catch((err) => console.warn('[vault] refresh error:', err.message));
  }, REFRESH_MIN * 60 * 1000).unref();
}

module.exports = {
  get ready() { return bootReady(); },
  hydrate,
  hydrateWithRetry,
  vault,
  applyBundle
